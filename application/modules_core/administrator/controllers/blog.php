...
class blog extends MX_Controller {
    ...
    public function save(){
        ...
        if ($this->form_validation->run() == FALSE){
          ...
          if(empty($_FILES['img']['name'])){
                ...
                $insert['tb_date_blog'] = date("Y-m-d");
                $insert['tb_source_blog'] = $this->input->post("content");
                $insert['tb_author_blog'] = $this->session->userdata("username");
            ...
            $config['upload_path'] = './media/blog/tmp/';
            ...
            if($this->upload->do_upload("img")){
                ...
                $source = "./media/blog/tmp/".$data['file_name'];
                $thumb = "./media/blog/".$data['file_name'];
                
                chmod($source, 0777);
                $this->load->library('image_lib');
                
                //Create Watermark Original Image
                $this->models_function->watermark($source,"blog");
                //Create Thumbnail
                $this->models_function->resize($thumb,"blog");
    ...
    public function update(){
        ...
        $a['madmin'] = $this->models_admin->menu("admin",  $this->perm_user);
        $a['profile'] = $this->models_admin->profile_top($this->session->userdata("id_user"));
        $a['action'] = $this->perm_user."/blog/updatesave";
        $whre['tb_id_blog'] = $uri;
